...
double SCManager::distDirectSC ( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    for ( int col_idx = 0; col_idx < _sc1.cols(); col_idx++ )
    {
        ...
        sum_sector_similarity = sum_sector_similarity + sector_similarity;  //cosine similarity
        num_eff_cols = num_eff_cols + 1;
    }
    
    double sc_sim = sum_sector_similarity / num_eff_cols;   //similarity normalization
    return 1.0 - sc_sim;    //to get local minimum
...
int SCManager::fastAlignUsingVkey( MatrixXd & _vkey1, MatrixXd & _vkey2)
{
    ...
    for ( int shift_idx = 0; shift_idx < _vkey1.cols(); shift_idx++ )
    {
        ...
        {
            argmin_vkey_shift = shift_idx;  //circshift를 통해 similarity의 local minimum이 가장 작은 shift index 찾음
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    MatrixXd desc = NO_POINT * MatrixXd::Ones(PC_NUM_RING, PC_NUM_SECTOR);  //20 Rings, 60 Sectors
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        ...
        if( azim_range > PC_MAX_RADIUS )
            ...
            //pt가 어느 sector, ring에 해당되는지 indexing
